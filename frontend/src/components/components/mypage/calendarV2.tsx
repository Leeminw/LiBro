...
const Cal = () => {
    ...
    const {data: imageDates, isSuccess, isLoading, isError, refetch} = useQuery<ImageDates>({
        queryKey: ['myCompleteBooks'],
        queryFn: () => Calendar.date(CurrentDateYear, CurrentDateMonth),
    });

    const countBooksForMonth = imageDates && Object.keys(imageDates).reduce((acc, date) => {
                    ...
                    {week.map(({day, otherMonth}, dayIndex) => {
                        ...
                        return (
                            ...
                            >
                                {imageCount > 1 && (
                                    <div
                                        className="absolute right-0 top-0 bg-[#F87171] text-white rounded-full text-xs w-5 h-5 flex items-center justify-center border border-white">
                                        {imageCount}
                                    </div>
