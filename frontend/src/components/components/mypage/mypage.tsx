...
export default function Myinfo() {
  ...
  const [userInfo, completeRatio, bookReviews, writtenComment] = results.map(
    (result) => result.data
  );

  const reviewList =
    bookReviews &&
    bookReviews.map((r: { rating: number }) => r.rating).filter((r: null | number) => r !== null);
    ...
    isSuccess && (
      <>
        <div className="mt-4 pb-3 flex w-full border-b border-gray-300">
          <div className="w-1/3 flex justify-center items-center">
            <div>
              ...
              <Dialog>
                <DialogClose className="" onClick={closeDialog}></DialogClose>
                <DialogTrigger asChild onClick={openDialog}>
                  <Button className="mt-4 bg-[#9268EB] text-white font-bold text-sm hover:bg-[#bfa1ff] hover:text-white">
                    프로필 수정
                  </Button>
                </DialogTrigger>
                <DialogContent className="p-4">
                  <DialogHeader className="pl-2 font-semibold text-base font-sans">
                    프로필 수정
                  </DialogHeader>
                  <div className="flex justify-center items-center w-full">
                    <div className="relative">
                      ...
                      <Avatar className="h-16 w-16 mt-2">
                        <AvatarImage src={tempProfile} alt="@defaultUser"></AvatarImage>
                        <AvatarFallback></AvatarFallback>
                      </Avatar>
                      <Button
                        className="absolute bottom-0 right-0 bg-[#9268EB] rounded-full p-0.5 w-6 h-6 hover:bg-[#462e79]"
                  ...
                  <div className="mx-10">
                    <Input
                      className="border-white border-b-black text-center text-gray-900 font-bold rounded-none"
                  ...
                  <DialogFooter>
                    <DialogClose>
                      <Button
                        type="submit"
                        onClick={submitEdit}
                        className="bg-[#9268EB] hover:bg-[#bfa1ff]"
                      >
                        수정하기
                      </Button>
                    </DialogClose>
          ...
          <div className="w-2/3 justify-center mt-2">
            <div className="mb-4 grid grid-cols-12 gap-4 pl-2">
            ...
            <div className="mb-3 grid grid-cols-12 gap-4 pl-2">
            ...
            <div className="flex justify-end">
              <Button
                className="aspect-square mr-1 w-28 min-w-20 bg-red-400 hover:bg-red-300"
                onClick={() => {
                  deleteUserInfo();
                  router.push("/login");
                  toast({
              ...
              >
                <MdLogout size={"1.2rem"} className="mr-1" />
                로그아웃
              </Button>
            </div>
          </div>
        ...
        <div className="flex flex-col">
          ...
          <div className="grid grid-cols-3 items-center mt-3 mb-6 pb-3 pt-3 border-b border-t border-gray-300">
            ...
            <div className="flex flex-row items-center justify-center space-x-2 col-span-1 border-gray-300">
        ...
        <div className="ml-2 mr-2 mt-4">
            ...
            <div className="flex items-center">
              <div className="mr-2 text-m font-bold text-end">
                {reviewCount === 0
                  ? 0
                  : (
                      reviewList.reduce((acc: number, rating: number) => acc + rating, 0) /
                      reviewCount
                    ).toFixed(1)}{" "}
                점
              </div>
              {renderStars(
                reviewList.reduce((acc: number, rating: number) => acc + rating, 0) / reviewCount
              )}
            </div>
            <div className="flex items-center">
          ...
          {[5, 4, 3, 2, 1].map((rating: number) => {
            const ScorePerRating: number =
              (reviewList.filter((r: number) => r === rating).length / reviewCount) * 100;
            return (
